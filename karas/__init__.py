...
from .Sender import Friend, Group, Member, Stranger
from .messages import MessageBase
from .event import Auto_Switch_Event, EventBase, RequestEvent, Event, NudgeEvent
from .elements import ElementBase, File, FlashImage, Image, Plain, Source, Voice
from .exceptions import InvalidArgumentError, VerifyError
from .util.Logger import Logging
from .util.network import error_throw, URL_Route, send_json
...
class Yurine(object):
    ...
    async def _initialization(self) -> None:
        ...
        if not self.session:
            ...
            self.logging.debug(f"get verifyKey {self.sessionKey}")
    ...
    async def _connect(self) -> Optional[int]:
        ...
        self.ws = await self.session.ws_connect(url=self.route("all"),
                                                headers={"verifyKey": self.verifyKey, "qq": str(self.account)})
        ...
        if _verify_data.get("code") != 0:
            raise VerifyError
        self.sessionKey: str = _verify_data["session"]
    ...
    def listen(self, registerEvent: Union[str, "EventBase", "MessageBase"]) -> None:
        """事件装饰器
        Args:
    ...
    async def _element_check(self, element: "ElementBase", type: str):
        if isinstance(element, (Image, Voice)):
            await self.uploadMultipart(element, type=type)
        return element.element

    @error_throw
    async def send_group(self, group: Union[int, "Group"],
                         Elements: Union[List[Union[ElementBase, MessageChain]], MessageChain],
                         quote: Union[int, Source] = None) -> int:
        """发送群组消息
        ...
        """
        _chain = [
            (await self._element_check(_e, type="group")) for _e in Elements] \
        ...
        self.logging.info(f"=> {[str(_e) for _e in _chain]}")
        await self.ws.send_json(
            send_json(
        ...
        _response = await self.ws.receive_json()
        return _response.get("messageId")
    ...
    async def send_nudge(self,
                         ...
                         nudgeEvent: "NudgeEvent" = None, ) -> None:
        """发送头像戳一戳消息
        ...
        _response: dict = await self.ws.receive_json()
        if _response.get("data").get("code") != 0:
            self.logging.error(f"{_response}")
        return _response.get("data").get("code")
